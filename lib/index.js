...
function makeUtterance (intentName, utterance, fileName) {
    ...
    fs.appendFile(fileName, output, function(err) {
        ...
        return console.log("Appended utterances to " + fileName + "\n");
    });
}
...
function stitchVar (fronts, term) {
    ...
    for (var front in fronts) {
        ...
        for (var i = 0; i < term.num; i++) {
            var curr = current;
            curr += "{" + term.words.slice(0, (i+1)).join(" ") + "|" + term.name + "}";
            newFronts.push(curr);
        }
    }
...
function splitUtterance (utterance) {
    ...
    while (cont) {
        ...
        var matches = remaining.match(newPattern);
        var uttText = {};
        var uttVar = {};
        if (matches === null) {
            uttText.type = "text";
        ...
        uttVar.type = "var"
        uttVar.words = matches[2].split(" ")
        // uttVar.num = Number.parseInt(matches[2]);
        uttVar.num = uttVar.words.length;
